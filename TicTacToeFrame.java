...
//					System.out.println("Index of button being clicked: " + rowIndex + " " + colIndex);
		...
		if (grid.isWinning(grid.getPlayer1Marker())) {
			int userOption = JOptionPane.showConfirmDialog(null,
									"Player 1 Wins! Another Game?",
									"Game Over", JOptionPane.YES_NO_OPTION);
			if (userOption == 0) {// 0 means Yes
				grid.initializeBoard();
				this.initializeBoard();
			} else if (userOption == 1) {// 1 means No
				System.exit(0); 
			}
		} else if (grid.isWinning(grid.getPlayer2Marker())) {
			int userOption = JOptionPane.showConfirmDialog(null,
					"Player 2 Wins! Another Game?",
					"Game Over", JOptionPane.YES_NO_OPTION);
			if (userOption == 0) {// 0 means Yes
				grid.initializeBoard();
				this.initializeBoard();
	...
	private void initializeBoard() {
		// See all cell texts to the empty cell marker.
		for (int row = 0; row < board.length; row++) {
			for (int col = 0; col < board[row].length; col++) {	
